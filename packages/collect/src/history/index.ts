...
export async function readHistories(host: Host, agents: readonly AgentEntry[] = CATALOG_AGENTS, onAgent?: (h: RecipeHistory) => void): Promise<AgentHistory[]> {
  ...
  for (const a of agents) {
    ...
    if (a.history === undefined) h = { agent: a.id, state: "no-reader", sessions: 0, calls: 0, usage: EMPTY };
      ...
      try {
        const usage = await tally(HISTORY_READERS[a.history.format].read(host, expand(host, a.history.root)));
...
export { type Count, type Usage, tally } from "./tally.js";
