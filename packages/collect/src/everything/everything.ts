...
import { place, programs, key } from "./location.js";
...
export interface EverythingOptions {
  ...
  clock?: () => number;
  /** Command line tools the tools rung found, by name; a config-location entry named for one is its config. */
  tools?: Iterable<string>;
  /** macOS apps the tools rung found (casks, App Store apps); a config-location entry named for one is its data. */
...
function row(id: string, name: string, kind: Kind, paths: string[], t: Tree, measured: Measured, extra: Partial<Pick<Row, "owner" | "binary" | "tool" | "linkTarget" | "excludes">> = {}): Row {
  ...
  if (extra.tool !== undefined) r.tool = extra.tool;
...
/** `<app dir>/<path inside it>`: the real path of what was split out of a record or found inside it, from the record's directory down; a HOME dotfile keeps its own name. */
...
export async function everything(m: Machine, opts: EverythingOptions = {}): Promise<Everything> {
  ...
  const known = await programs(m, prov, opts.tools ?? [], opts.apps ?? []);
  ...
  for (const p of pairs) {
    ...
    const extra = { owner: p.owner, binary: p.binary === undefined ? undefined : tilde(m.home, p.binary.path), tool: p.name, linkTarget: p.dirs[0]?.linkTarget, excludes: p.dirs.flatMap(d => d.excludes).sort() };
    const primary = paths[0];
    drafts.push({ row: row(p.name, primary === undefined ? p.name : basename(primary), "config", paths, tree, measured, extra), named: true, dirs: p.dirs });
  }

  for (const d of rest) {
    const hit = d.role === "unknown" && d.kind === "dir" ? lookup(tilde(m.home, d.path))[0]?.app : undefined;
    const where = place(m.home, d.path, d.kind, known);
    const kind: Kind = hit !== undefined ? "config" : where?.kind === "app-data" ? "app-data" : d.role === "unknown" && where !== undefined ? where.kind : d.role;
    const tool = hit ?? (d.role === "unknown" || where?.kind === "app-data" ? where?.tool : undefined);
    const owner = d.role === "unknown" && where?.kind === "config" ? where.owner : undefined;
    const binary = d.role === "unknown" && where?.kind === "config" && where.binary !== undefined ? tilde(m.home, where.binary) : undefined;
    const carved = d.paths[0] !== undefined && d.paths[0] !== d.path;
    const name = !carved ? basename(d.path) : d.paths.length > 1 ? carveName(m.home, d) : inside(m.home, [d], d.paths[0] ?? d.path);
    const r = row(d.path, name, kind, d.paths, d, d.measured, { linkTarget: d.linkTarget, excludes: d.excludes, tool, owner, binary });
    if (kind === "unknown" && d.measured === "exact" && d.mtime > 0 && now - d.mtime > TWO_YEARS) flag(r, "stale");
    drafts.push({ row: r, named: false, dirs: [d] });
  }
  ...
  // A binary the tools rung already has a row for is that row's; a second, locked row here would only repeat it.
  const listed = (name: string): boolean => known.tools.has(key(name));
  for (const l of prov.leftovers) if (!paired.has(l.name) && !listed(l.name)) drafts.push(leftover(l));
  ...
  const kept = drafts.flatMap(d => {
    ...
    if (r.binary === undefined || listed(r.tool ?? r.name)) return [];
  ...
  // Named last, for the paths a row still holds once credentials and claims have come off it: a pair for its first path, a carve for what it holds.
  for (const d of kept) {
    const rec = d.dirs[0];
    const primary = d.row.paths[0];
    if (rec === undefined || primary === undefined || d.row.kind === "credential") continue;
    if (d.row.tool !== undefined && d.row.kind === "config") d.row.name = basename(primary);
    if (d.dirs.length > 1 || rec.role === "unknown" || primary === rec.path) continue;
    d.row.name = d.row.paths.length > 1 ? carveName(m.home, { ...rec, paths: d.row.paths }) : inside(m.home, [rec], primary);
  }
  for (const d of kept) {
    const primary = d.row.paths[0];
