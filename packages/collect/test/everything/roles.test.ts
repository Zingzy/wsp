...
import { roleByName, roles } from "../../src/index.js";
import { HOME, OLD, home, laptop } from "./fixture.js";
...
describe("pass 2: directory role", () => {
  it("marks cache and state by spec root and by subdirectory name, splitting marked subtrees out of their parent", async () => {
    ...
    expect(dirs.map(d => [rel(d.path), d.role, d.files, d.paths.map(rel)])).toEqual([
      ...
      ["~/.local/state", "state", 1, ["~/.local/state"]],
      ...
      ["~/Library/Caches", "cache", 1, ["~/Library/Caches"]],
    ]);
  });
  ...
  it("fixture sizes after the split", async () => {
    const dirs = await roles(laptop(home()));
    const raycast = dirs.find(d => d.path === `${HOME}/.config/raycast` && d.role === "unknown");
    const extensions = dirs.find(d => d.path === `${HOME}/.config/raycast` && d.role === "state");
    expect(raycast).toMatchObject({ bytes: Buffer.byteLength('{"theme":"dark"}'), files: 1 });
    expect(extensions).toMatchObject({ bytes: 3_000_000, files: 1 });
    expect(dirs.find(d => d.path === `${HOME}/.cache`)).toMatchObject({ bytes: 5_000_000 + Buffer.byteLength("hf_redacted"), files: 2 });
  });
  ...
  it("Library/Caches is a macOS root only", async () => {
    const dirs = await roles(laptop({ ...home(), platform: "linux" }));
    expect(dirs.some(d => d.path.endsWith("/Library/Caches"))).toBe(false);
  });

  it("names that mean cache or state at any depth", () => {
    expect(["node_modules", ".venv", "virtenv", "logs", "extensions", "installs", "versions", "builds", "projects", "sessions", ".git", ".zsh_history", ".zsh_sessions"].map(roleByName)).toEqual(Array<string>(13).fill("state"));
