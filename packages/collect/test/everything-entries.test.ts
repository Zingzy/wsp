...
describe("entriesFor: one manifest row per found row", () => {
  const cases: { row: Row; want: Record<string, unknown> }[] = [
    ...
    },
    {
      ...
      want: { group: APP_DATA_GROUP, role: "app-data", detail: "data of the macOS app Arc; nothing on a Linux machine reads it" },
    },
    {
      ...
      want: { group: APP_DATA_GROUP, detail: "data of a macOS app; nothing on a Linux machine reads it; large; never copied without a tick" },
    },
    {
  ...
  it("groups by location, then by app directory in HOME when two or more rows share one, then app data, large and what never copies", () => {
    const entries = entriesFor([
      ...
      row({ id: ".aside/state", name: "state files in ~/.aside: logs, sessions", kind: "state", paths: ["~/.aside/logs", "~/.aside/sessions"], flags: ["large"] }),
    ...
    expect(entries.map(e => [e.label, e.group])).toEqual([
      ...
      ["Caches", "~/Library"],
      ["Arc", APP_DATA_GROUP],
      ["Arc/Cache", APP_DATA_GROUP],
      [".aaa-big", LARGE_GROUP],
      [".config/raycast/extensions", LARGE_GROUP],
      [".zz-big", LARGE_GROUP],
      ["state files in ~/.aside: logs, sessions", LARGE_GROUP],
