...
import { CLAUDE_KEY_FILE } from "@wsp/engine";
import { CONFIG_DIR } from "./doctor.js";
...
export const CLAUDE_KEY_PATH = `${CONFIG_DIR}/${CLAUDE_KEY_FILE}`;
...
export const SIGN_INS: Readonly<Record<string, SignIn>> = {
  ...
  gh: { kind: "command", sources: ["file", "keychain"], login: "gh auth login", status: { command: "gh auth status", signedIn: o => /Logged in to/.test(o) && !/Failed to log in/.test(o) }, toolTimeoutMs: 15 * MIN },
  ...
  vercel: { kind: "command", sources: ["file"], login: "vercel login", status: { command: "vercel whoami", signedIn: (o, c) => c === 0 && !/No existing credentials/.test(o) }, toolTimeoutMs: 15 * MIN },
  netlify: { kind: "command", sources: [], login: "netlify login", status: { command: "netlify status", signedIn: (o, c) => c === 0 && !/Not logged in/i.test(o) }, toolTimeoutMs: 5 * MIN },
  fly: { kind: "command", sources: [], login: "fly auth login", status: { command: "fly auth whoami", signedIn: ok(/@/) }, toolTimeoutMs: 15 * MIN },
  supabase: { kind: "command", sources: [], login: "supabase login", fallback: "supabase login --no-browser", status: { command: "supabase projects list", signedIn: ok() } },
  railway: { kind: "command", sources: [], login: "railway login", status: { command: "railway whoami", signedIn: ok(/Logged in as/) }, toolTimeoutMs: 5 * MIN },
  doppler: { kind: "command", sources: [], login: "doppler login", status: { command: "doppler me", signedIn: ok() }, toolTimeoutMs: 5 * MIN },
  ...
  codex: { kind: "command", sources: ["file"], login: "codex login", fallback: "codex login --device-auth", status: { command: "codex login status", signedIn: ok(/Logged in using/) } },
  gemini: { kind: "command", sources: ["file"], login: "gemini", status: { command: GEMINI_STATUS, signedIn: ok(), detail: geminiSource }, toolTimeoutMs: 5 * MIN },
  ...
  cloudflared: { kind: "command", sources: ["file"], login: "cloudflared tunnel login" },
  ...
  pi: { kind: "command", sources: ["file"], login: "pi", status: { command: "pi --list-models", signedIn: ok(/^provider\s+model\b/m) }, note: "type /login inside pi and pick a provider, then /exit; a key on the machine counts" },
  // The pool lists keys from ~/.hermes/.env and the environment beside stored logins; with none it prints nothing on exit 0.
  hermes: { kind: "command", sources: ["file"], login: "hermes auth", status: { command: "hermes auth list", signedIn: ok(/\(\d+ credentials\):/), detail: secretNamed }, note: "pick Add a credential in the menu; keys in ~/.hermes/.env count" },
};
