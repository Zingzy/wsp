...
import { catalogEntry, CLAUDE_CONFIG_DIR, formulaCommand, GOLDEN_SETUP, GOLDEN_SMOKE } from "@wsp/catalog";
...
/** The commands the recipe's ticked tools answer by, for the read from inside a workspace below: a catalogue row
 * by the command its own entry names, and a formula by the command a formula puts on PATH, which is the rule the
 * road that takes a tap formula off again reads too. A row filed under another manager is left out: its package
 * name is not its command (`@openai/codex` is `codex`), and a guess would fail a workspace for a name nothing
 * installed. */
function tickedCommands(recipe: Pick<RecipeDigest, "ticks"> | undefined): string[] {
  const named = (recipe?.ticks ?? []).flatMap(tick => {
    if (tick.id.startsWith(CATALOG_PREFIX)) {
      const entry = catalogEntry(tick.id.slice(CATALOG_PREFIX.length));
      return entry?.kind === "tool" && entry.bin !== undefined ? [entry.bin] : [];
    }
    return tick.id.startsWith(BREW_ID_PREFIX) ? [formulaCommand(tick.id.slice(BREW_ID_PREFIX.length))] : [];
  });
  return [...new Set(named)].sort();
}

/** The tools the recipe ticks, read from inside the workspace this run made: one `command -v` per row, on the one
 * PATH a machine's tools sit on. The read the spoo proof of 2026-09-18 had to be done by hand, where gh and every
 * other Homebrew row was on the box and in no workspace of it, because the prefix that road installs into is
 * outside the trees a workspace carries. A recipe that ticks nothing this can name has nothing to read and says so.
 *
...
export async function recipeToolsInside(machine: Pick<Machine, "exec">, recipe: Pick<RecipeDigest, "ticks"> | undefined): Promise<string> {
  ...
  if (bins.length === 0) return "the recipe ticks no tool this can name a command for, so there is nothing to read inside";
