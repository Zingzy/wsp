 ...
 * it and that it is on this Mac; for any other, the manager's install line and that it runs after the catalog. */
export function alsoItems(rows: readonly ScanRow[], recipe: Recipe, build: (row: ScanRow) => string | undefined = () => undefined): SelectItem[] {
  return rows.map(r => {
    const tone = sizeTone(r.size);
    const road = ownRowOf(recipe, r) !== undefined ? build(r) : undefined;
    const detail = road !== undefined ? [road, installedOnMacLine(r.version)] : [r.install, r.version === undefined ? "installs on the machine after everything in the catalog" : `${r.version} here; installs on the machine after everything in the catalog`];
    return {
      id: r.id,
      label: r.name,
      group: r.group,
...
export function scannedTicks(recipe: Recipe, rows: readonly ScanRow[]): Set<string> {
  ...
  return new Set(rows.filter(r => ownRowOf(recipe, r)?.on ?? carried.has(customFromScan(r).id)).map(r => r.id));
}
