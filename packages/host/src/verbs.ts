  ...
  },
  {
    ...
    run: async ctx => {
      ...
      const forksNothing = (word: string, kind: WorkspaceKind): void => {
        if (from !== undefined || size !== undefined) throw usageRefusal(`${word} forks nothing, so it takes no --from or --size.`, "Drop them.");
    ...
    tool: tool({
      ...
      call: async ({ name, from, size: word, local, ssh, ssh_port: sshPort, ssh_key: sshKey, spawn, max_machines: maxMachines, max_depth: maxDepth }, deps) => {
        ...
        if (local === true || ssh !== undefined) {
          if (from !== undefined || word !== undefined) throw usageRefusal("a workspace on a machine that already exists forks nothing, so it takes no from or size.", "Drop them.");
