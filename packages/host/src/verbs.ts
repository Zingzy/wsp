...
export function placeLines(places: readonly PlaceView[]): string[] {
  ...
  return table([["PLACE", "KIND", "CORES", "MEMORY", "DISK FREE", "WORKSPACES", "ENGINE", "PRESENT", "FORKS", "LAST SEEN", "DEFAULT", "BEHIND", "IMAGE"], ...rows]);
}
...
export const VERBS: readonly Verb[] = [
  ...
  },
  {
    ...
    run: async ctx => {
      ...
      ctx.out.emit({ turn: turnView(await followVerb(ctx, client, openingOf(ctx.env, created.workspace, task, { harness, ...picks, cwd: flag(ctx.flags, "cwd"), notify }), true, {}, { spend: turnSpendWord(created.workspace) })) });
      return 0;
    },
    tool: tool({
