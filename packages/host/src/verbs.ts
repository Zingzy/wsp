...
async function followVerb(ctx: VerbContext, client: HostClient, start: Record<string, unknown>, announce: boolean, picks: Picks = {}, opened?: (threadId: string) => string, onTurn?: (turn: Turn) => void): Promise<Turn> {
...
export const VERBS: readonly Verb[] = [
  ...
  {
    name: "new",
    usage: "wsp new <name> [--on <place>] [--from <project golden>] [--size <cpu>x<memGb>] [--engine] [--spawn on|off] [--max-machines <n>] [--max-depth <n>]",
  ...
  },
  {
    ...
    run: async ctx => {
      ...
      try {
        ...
        else ctx.out.emit(turnView(await followVerb(ctx, client, opening, true, {}, opened, t => (started = t))));
      } catch (e) {
