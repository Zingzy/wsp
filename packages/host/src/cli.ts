...
import { ANTHROPIC_KEY, agentKeyEnvs, keyIn, parseEnvFile, savedEnv, type Keys } from "./env-keys.js";
...
export function swapProvider(rt: Runtime, keys: Readonly<Record<string, string | undefined>>): void {
  ...
  slot.swap(providerBackendFor({ ...(PROVIDER_ENVS.get(rt) ?? process.env), ...keys }));
}
