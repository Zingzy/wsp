...
describe("wsp recipe", () => {
  ...
  it("writes the recipe where asked, creating the directory, and prints the agents and the tools as tables", async () => {
    ...
    expect(line(/Node 22/)).toMatch(/^● {2}Node 22 with npm\s+base\s+always on the image\s+198\.8 MB$/);
    expect(line(/Java 21/)).toMatch(/^○ {2}Java 21\s+catalog\s+in the catalog, on request\s+584\.9 MB$/);
    expect(line(/GitHub CLI/)).toMatch(/^● {2}GitHub CLI\s+installed\s+installed here, never used\s+40\.2 MB$/);
    // The one row the catalog could not measure says so, instead of being folded into the estimate's unmeasured count.
    expect(line(/1Password CLI/)).toMatch(/^○ {2}1Password CLI\s+catalog\s+in the catalog, on request\s+size unknown$/);
    expect(lines.filter(l => /^On: /.test(l))).toEqual([expect.stringMatching(/^On: 1 agent, 208\.0 MB$/), expect.stringMatching(/^On: \d+ tools, [\d.]+ [MG]B$/)]);
