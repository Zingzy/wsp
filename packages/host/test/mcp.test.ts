...
describe("the MCP server over the host", () => {
  ...
  it("offers the verbs as tools, each described", async () => {
    ...
    expect(tools.map(t => t.name).sort()).toEqual(["delete", "exec", "export", "folders", "forget", "fork", "import", "new", "pause", "projects", "rebuild", "recipe", "recipe_scan", "rename", "send", "snapshot", "stop", "terminal_config", "thread_new", "thread_read", "thread_rename", "threads", "threads_wait", "wake", "workspaces"]);
