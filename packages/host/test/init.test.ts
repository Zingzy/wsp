...
describe("wsp init, the project the run is for", () => {
  it("with no folder named the first screen asks for one, and what it answers is read, carded and grouped on the tools screen", async () => {
    ...
    await f.press(KEY.enter);
    await f.until("What they need");
    // Go is off in the catalog and never used here; the folder's own go.mod put it on the machine.
    expect(f.text()).toContain("1 your project needs");
    f.clear();
    await f.press("a");
    await f.until("space tick");
    expect(f.text()).toMatch(/Your project needs[\s\S]*● Go\s+251\.0 MB/);
    await f.press(KEY.enter);
    for (const screen of ["What they need", "Sign-ins and keys"]) {
      await f.until(screen);
      await f.press(KEY.enter);
    }
    await f.until(BOOT);
