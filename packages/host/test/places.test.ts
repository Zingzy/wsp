...
import { ALREADY_JOINED_LINE, DAEMON_VERSION, JOIN_NO_KEY_REFUSAL, PLACE_ADD_WORDS, PLACE_CODE_REFUSAL, PLACE_DOOR_UNSERVED, PlaceReport, doorPortHeldLine, joinKeyRefusal, joinToken, placeDaemonBehind, placeDaemonPaths, placeLinkTranscript, placeNoChipLine, placeOwnedPaths, placeUpdateLine, shellQuote, workFolderIn, wsUrlOf, type PlaceDoorView, type PlaceView } from "@wsp/protocol";
...
import { SERVICE_MANAGERS, serviceTag, type RunResult, type ServiceRunner } from "../src/service.js";
...
describe("wsp add <place> --update", () => {
  ...
  it("names the place by the word wsp places prints and asks the host to move it, then says both versions and the road", async () => {
    ...
    const io = captured();
    const fake = updateClient({ name: "spoo", from: 27, to: DAEMON_VERSION, road: "link", at: "/home/maya/.wsp/daemon/wsp-daemon" });
...
describe("which binary an update carries and how it travels", () => {
    ...
    return {
      ...
      link: {
        request: (op: string, params?: Record<string, unknown>) => {
          ...
          return Promise.resolve(params?.["last"] === true ? { at: "/home/maya/.wsp/daemon/wsp-daemon" } : {});
        },
      } as never,
    };
  };
  ...
  it("picks the binary by the chip that computer said it is, never this one's, and sends it as bytes under one upload id", async () => {
    ...
    expect(landed).toEqual({ road: "link", at: "/home/maya/.wsp/daemon/wsp-daemon" });
...
describe("the update over the ssh road, where the link is down", () => {
  ...
  const UNIT = placeUnitName(HOME);

  it("names the unit the join wrote, off the one path every file wsp keeps on a place sits under", () => {
    expect(UNIT).toMatch(/^wsp-place-[0-9a-f]{8}\.service$/);
    // Both sides read the rule off the place file's own path, so neither guesses the other's name.
    expect(UNIT).toBe(`wsp-place-${serviceTag(placeDaemonPaths(HOME).placeFile)}.service`);
  });
  ...
  it("moves the landed binary over the path the unit itself names, keeps the old one and restarts, sweeping nothing", () => {
    ...
    expect(script).toContain(`systemctl --user show -p ExecStart --value ${shellQuote(UNIT)}`);
    expect(script).toContain("PLACE_NO_UNIT");
    expect(script).toContain(`cp -f "$exe" "$exe.old"`);
    ...
    expect(script).toContain(`systemctl --user restart ${shellQuote(UNIT)}`);
    // The port file is the proof the new daemon bound, so the old one is taken before the restart is waited on.
    expect(script.indexOf(`rm -f ${shellQuote(placeDaemonPaths(HOME).portFile)}`)).toBeGreaterThan(script.indexOf("restart"));
