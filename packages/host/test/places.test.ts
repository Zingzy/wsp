...
import { ALREADY_JOINED_LINE, PLACE_ADD_WORDS, PLACE_CODE_REFUSAL, PLACE_DOOR_UNSERVED, PLACE_NEEDS_ROOT_LINE, PlaceReport, doorPortHeldLine, placeDaemonPaths, placeLinkTranscript, shellQuote, workFolderIn, wsUrlOf, type PlaceDoorView, type PlaceView } from "@wsp/protocol";
...
describe("a computer joining a wsp", () => {
  ...
  it("refuses a login that is not root with one sentence and writes nothing, since the agent is the machine's service", async () => {
    ...
    const said = await joinCommand(io, [host.url], { code: "7QK3M2VD", name: "old-macbook" }, deps).then(() => "", (e: unknown) => String(e));
...
describe("which manager holds the agent's unit", () => {
  it("takes the manager the platform has, and refuses a computer that has none before it writes or dials anything", async () => {
    ...
    const own = captured();
    const runner = fakeRunner();
    await joinPlace(own, { home, addresses: [host.url], code: "B", platform: "linux", uid: 0, manager: unitsUnder(home), run: runner.run, dial: url => new WebSocket(wsUrlOf(url)) });
