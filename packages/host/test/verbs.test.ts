...
describe("wsp verbs over the host", () => {
  ...
  describe("one list behind every verb", () => {
    ...
    it("every verb takes the workspaces the caller's own listing prints, by name and by id", async () => {
      ...
      // A send names a thread of this caller's own tree, so the thread it opens here is the one it then sends into.
      expect((await line("run", "alpha", "a builder")).code).toBe(0);
      const mine = (await rt.sessions.list()).find(v => v.parentThreadId === "t_lead")!;
      expect((await line("send", mine.threadId!, "and the rest")).code).toBe(0);
    });
