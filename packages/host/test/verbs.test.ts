...
describe("wsp verbs over the host", () => {
  ...
  it("every verb takes --json and --help; a bad flag prints the usage", async () => {
    ...
    expect(foreign.io.errors).toEqual(['--agent belongs to wsp skills add, wsp servers signin, wsp servers tools, wsp fork and wsp run; wsp send does not read it. usage: wsp send <thread> [--model, --effort <value>] [--image <path>] [--detach] "<message>"']);
