...
import { SolariBackend, diffRecipes, killUntilGone, type GoldenStage, type Machine } from "@wsp/engine";
...
describe.runIf(LIVE)("golden update (live: v1, v2 by fork, v3 on the kept builder, forks proved)", () => {
  ...
  const runtimeFor = (rows: readonly ManifestEntry[], frames: Frame[], t0: number): { rt: Runtime; imp: GoldenImport } => {
    ...
    const rt = createRuntime({
      ...
      adapters: {},
      goldenRecipe: goldenRecipeFor(rows, { anthropic: env.ANTHROPIC_API_KEY }, { import: imp, deployDaemon: async m => `node ${(await deployDaemon(m)).node}` }),
