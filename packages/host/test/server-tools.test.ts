...
function fixture(): Fixture {
  ...
  mkdirSync(home);
  mkdirSync(bin);
  for (const [name, text] of Object.entries({ server: SERVER, mute: MUTE, crash: CRASH, claude: CLAUDE })) {
    writeFileSync(join(bin, name), text);
    chmodSync(join(bin, name), 0o755);
  }
  return { home, bin, config: servers => writeFileSync(join(home, ".claude.json"), JSON.stringify({ mcpServers: servers })) };
}

/** This computer's own Host over the fixture's home and PATH. */
function here(f: Fixture): Host {
  const live = nodeHost();
  return { ...live, home: f.home, exec: { ...live.exec, run: (cmd, args, o) => live.exec.run(cmd, args, { ...o, env: { PATH: `${f.bin}:/usr/bin:/bin`, HOME: f.home, ...o?.env } }) } };
}
...
describe("one MCP server's tools, on the person's ask", () => {
  ...
  it("stops a server that has not answered when the time is up, with its process group, and asks it again on the next click", async () => {
    ...
    expect(await reader.tools({ kind: "here" }, { key: "here", agent: "claude", name: "crash" })).toMatchObject({ auth: "failed", refused: "airtable: AIRTABLE_API_KEY is not set" });
