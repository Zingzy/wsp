...
describe("the init job view", () => {
  ...
  it("the event rides the events channel beside the runtime's own, and the ops parse as requests", () => {
    ...
    expect(RuntimeRequest.safeParse({ id: 9, op: "init.signInCode", tool: "gcloud", code: "" }).success).toBe(false);
    expect(RuntimeRequest.safeParse({ id: 10, op: "init.signInCode", tool: "gcloud", code: "x".repeat(SIGN_IN_CODE_MAX + 1) }).success).toBe(false);
  });
});
...
describe("the words the clients print for the job", () => {
  ...
  it("the sidebar button's facts: the rows over as a fraction of the total, whether the person is waited on, and its words", () => {
    ...
    const waiting = { ...JOB, phase: "signing-in" as const, progress: { done: 2, total: 4 }, rows: [...JOB.rows, row({ id: "sign-in/gh", kind: "sign-in", label: "GitHub CLI login", state: "open", page: "https://github.com/login/device", code: "8F4A-C21B" })] };
  ...
  it("a sign-in row names the road it finishes on, so a client knows which row takes a code from the person", () => {
    ...
    const takesCode = row({ id: "sign-in/gcloud", kind: "sign-in", tool: "gcloud", label: "Google Cloud login", state: "open", page: "https://accounts.google.com/o/oauth2/auth", finish: "code" });
