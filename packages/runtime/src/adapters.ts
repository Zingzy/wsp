...
import { CATALOG_AGENTS, mintsToken, type ThreadAgent } from "@wsp/catalog";
...
/** What the vault holds for one agent, under the variables that agent's own row declares: the token it mints and
 * the key it reads. One reader for every agent, so a new one is a row and not a branch here. */
export function secretsOf(vault: Readonly<Record<string, string>>, id: ThreadAgent): { oauthToken?: string; apiKey?: string } {
  ...
  const key = "keyEnv" in signIn && signIn.keyEnv !== undefined ? vault[signIn.keyEnv] : undefined;
  return { ...(token !== undefined ? { oauthToken: token } : {}), ...(key !== undefined ? { apiKey: key } : {}) };
}
