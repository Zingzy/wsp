  ...
  const fork = (record: WorkspaceRecord, bind: (machine: Machine) => void, override?: WorkspaceSpec, report?: StageReport): Promise<Machine> =>
    claiming(
      ...
      async b => {
        ...
        if (named.refused === undefined) report?.("hostname-set", `Hostname set to ${named.host}.`);
