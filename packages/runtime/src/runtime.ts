...
export function createRuntime(opts: RuntimeOptions): Runtime {
  ...
  const projectsDoor = {
    ...
    async add(o: { source: string; on?: string; name?: string; base?: string; seed?: SeedChoice }, origin?: Caller): Promise<ProjectView & { notice?: string }> {
      ...
      // What is being added and where it comes from. What travels with it is the seeding's own line, in the counts
      // the menu showed, so this one counts nothing: the file count alone read as the whole of a seed that was
      // also carrying a memory folder.
      report("planned", `${project.name} from ${sourceWord(source)}${seed === undefined ? ", nothing seeded" : ""}.`);
      try {
