...
import { endLocalRuns, localExecStream, type GroupWorkReader } from "../src/local-exec.js";
...
describe("local exec stream", () => {
  ...
  it("a child that prints nothing while the tree it started burns a core is not cut at the idle limit", async () => {
    const factory = localExecStream({ root, idleMs: 400, deadlineMs: 30_000, pollMs: 20, readWork: burningTree(Date.now) });
