...
describe("one place for the size a machine is built at", () => {
  ...
  // module of a host with no provider, whose machines have no shape because there are none, the ssh module, whose
  // table offers no size either, since every machine it holds answers with its own, and the module that answers out
  // of memory, whose table is what a fixture's forks read as.
