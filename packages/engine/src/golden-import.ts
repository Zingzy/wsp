...
/** The release road of a tap row whose GitHub release is known, with the pin its first install recorded. */
const releaseRoad = (r: Pick<PlannedRoad, "source" | "pin">): InstallRoad => ({ road: "release", repo: r.source.repo, version: r.source.tag, ...pinOf(r) });

/** What a tools row installs by, with the command it puts on PATH where known: a catalog row its entry's road
 * (noted when no golden build has proven the road), a formula row the brew road, a manager row its manager's road
 * at the row's version; nothing for a row no road installs. A road carries the pin the row's first install recorded. */
