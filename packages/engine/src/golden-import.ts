...
import { APT_ENV, APT_INDEX, APT_UPDATE, BASE_FLOOR, BASE_IMAGE_COMMANDS, BREW, BREW_PREFIX, CATALOG_AGENTS, CATALOG_TOOLS, CLAUDE_KEY_FILE, CLAUDE_SETTINGS_FILE, HOMEBREW, HOMEBREW_STEP, NODE_PATH_LINE, NODE_RELEASES, LINUXBREW_SHIM, ROADS, UV_INSTALL, asLinuxbrew, asLinuxbrewScript, baseEntryFor, baseNote, catalogEntry, catalogToolFor, installLine, nodeInstallScript, pinStateOf, ROAD_MODULES, roadModule, smokeOf, standingPin, unpinned, type AgentEntry, type InstallRoad, type NodeMajor, type RoadName, type ToolPin } from "@wsp/catalog";
...
const FORMULA_PREFIX = "tools/brew/";
/** The formula a Homebrew row names; nothing for a tap or any other row. */
export const formulaOf = (e: Pick<RecipeEntry, "id">): string | undefined => (e.id.startsWith(FORMULA_PREFIX) ? e.id.slice(FORMULA_PREFIX.length) : undefined);
