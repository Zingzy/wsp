...
import { ALREADY_APPLIED, fmtBytes, goldenHead, type GoldenBaseTool, type GoldenLeftBehind, type GoldenLogin, type GoldenManifest, type GoldenMissingTool, type GoldenStage, type GoldenVersion, type RecipeDigest } from "@wsp/protocol";
...
/** The recipe rows a delta addresses: what it removes and what it plans again, tools and agents alike. */
function touchedBy(delta: GoldenDelta): Set<string> {
  return new Set([...delta.removals, ...delta.import.tools, ...(delta.import.skippedTools ?? []), ...delta.import.agents, ...(delta.import.skippedAgents ?? [])].map(t => t.id));
}
...
/** What an updated image is without: the previous version's notes for rows the delta neither removed nor planned
