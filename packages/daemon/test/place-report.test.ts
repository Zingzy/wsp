...
const reportHere = (): ReturnType<typeof placeSelfReport> => {
  const dir = home();
  writeFileSync(join(dir, "place.json"), placeFileText({ placeId: "p_1", name: "box", hostName: "mac", hostUrls: ["http://x"], hostPublicKey: "k", keyPath: join(dir, "k.pem"), joinedAt: new Date(0).toISOString(), awake: false }));
