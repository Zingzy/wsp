...
| `wsp thread new --in <workspace> [--agent <id>] [--model <slug>] [--effort <word>] [--access <word>] [--cwd <path>] [--notify <thread\|me>] [--title <name>] "<task>"` | `thread_new` (workspace, task, agent, model, effort, access, cwd, notify, title) | opens a thread and follows its first turn to the reply |
