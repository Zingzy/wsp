...
| `wsp send <thread> [--model <slug>] [--effort <word>] [--access <word>] [--detach] "<message>"` | `send` (thread, message, model, effort, access, detach) | a message into an existing thread; follows the turn to the reply, or with `--detach` returns the moment the turn is started |
