...
- Gate before every report: `pnpm -r --filter '!@wsp/desktop' build`, `pnpm test`, `pnpm -r exec tsc --noEmit`, `pnpm --filter @wsp/web build`. A red gate is your problem to fix, not to report around. A timing flake: rerun that file alone once, name it in the report.
