...
- Run test files from the repo root by path (`pnpm exec vitest run --maxWorkers=2 <path>`); never `pnpm --filter <pkg> test`, which runs the whole package with every core.
