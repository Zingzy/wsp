...
- Color law: green = running, ONLY; orange = spend/confirm, and the middle weight tier (disk 70 to 90 percent, sizes 300 MB to 1 GB), ONLY; yellow and red exist only as the tiers beside it (yellow from 100 MB, red from 1 GB and from 90 percent of the disk); the tables live in the protocol (`sizeTone`, `diskTone` in format.ts) and the app and the wizard both read them; everything else zinc. Terminal pane darker than chrome (content-well inversion) stays.
