...
The package is `wspx`; the command it puts on your path is `wsp`. To try it
once without installing, `npx wspx init` runs the same first run, but nothing
...
| `wspx` | what npm publishes: the host, the app and the daemon in one bundle |
