...
import { CLOUD_SETUP_WORDS, LOGIN_STATE_WORDS, SIGN_IN_OPEN_STATE, type InitJob, type InitRow } from "@wsp/protocol";
...
function screenWith(...rows: InitRow[]) {
  const onCode = vi.fn();
  const job: InitJob = { id: "init_1", road: "manual", phase: "signing-in", keys: { solari: true, anthropic: false }, screens: [], rows, progress: { done: 0, total: rows.length }, log: [] };
  render(<SetupBuild job={job} onHide={vi.fn()} onCancel={vi.fn()} onOpenWorkspace={vi.fn()} onAgain={vi.fn()} onCode={onCode} refusal={null} />);
...
describe("the code field under a sign-in row", () => {
  it("is on the row whose page hands a code back, under it, with the page's own keycap still there", () => {
    ...
    // Under the row it belongs to, not inside it.
    expect(build.querySelector('[data-row="sign-in/gcloud"] [data-k=code-field]')).toBeNull();
  ...
  it("is nowhere else: not on the callback road, not on a row with no road, not once the row is over", () => {
    for (const over of [{ finish: "callback" as const }, {}, { finish: "code" as const, state: LOGIN_STATE_WORDS["signed-in"] }]) {
