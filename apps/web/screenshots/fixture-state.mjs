 ...
 * points, as a host that has been metering holds them: the run's first tick and its newest, which is minutes old,
 * so the host carries the line on from there rather than starting again. */
const meter = (workspaceId, { rateUsdPerHour, hours, phase = "running" }) => {
  ...
  return [
    workspaceId,
    { workspaceId, points: [point(METERED_MIN, hours * 3_600_000 - (rate > 0 ? (METERED_MIN - 2) * 60_000 : 0), total - run), point(2, hours * 3_600_000, total)] },
  ];
};
