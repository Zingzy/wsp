...
const surfaceFrom = (raw, index, widths) => {
  ...
  const { name, at, steps, wait, settleMs, remote } = raw;
  ...
  for (const width of list.widths) {
    for (const theme of THEMES) {
      for (const surface of list.surfaces) {
        ...
        shots.push({ name: surface.name, at: surface.at, steps, wait: surface.wait, settleMs: surface.settleMs, remote: surface.remote, theme, width, height: list.heights[width], file: shotName(surface.name, theme, width) });
      }
    }
  }
  return shots;
...
export function indexMarkdown(list, written, meta) {
  ...
  for (const surface of list.surfaces) {
    ...
    const road = `Route \`${surface.at}\`${surface.remote ? ", served to a window on another computer" : ""}${surface.steps.length > 0 ? `, then ${surface.steps.map(s => `${stepWords(s)}${s.width === undefined ? "" : ` (at ${s.width} only)`}`).join(", ")}` : ""}.`;
