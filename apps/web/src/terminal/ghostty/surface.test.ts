// Adapted from pingdotgg/t3code apps/web/src/terminal/ghostty/surface.test.ts at 57a66608 (MIT).
...
describe("terminalLinkAtColumn", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://wsp.site", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe("https://wsp.site");
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe("https://wsp.site");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://wsp.site", false, true)];
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://wsp.site", false), row("", false)];
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe("https://wsp.site");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://wsp.dev", (character) => cell(character)),
      ],
      text: "🙂 https://wsp.dev",
      isWrapContinuation: false,
      wrapsToNext: true,
    ...
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe("https://wsp.site");
  });
});
