// Adapted from pingdotgg/t3code apps/web/src/components/ui/toast.tsx at 57a66608 (MIT).
"use client";
...
import { type ScopedThreadRef, type ThreadId } from "./types";
import {
  CheckIcon,
  ChevronDownIcon,
  ChevronUpIcon,
...
interface ToastProviderProps extends Toast.Provider.Props {
  ...
  /** Thread-scoped toasts render only while this thread is the active one. */
...
function ToastProvider({
  ...
  return (
    <Toast.Provider toastManager={toastManager} {...props}>
      {children}
      <Toasts position={position} activeThreadRef={activeThreadRef} />
    </Toast.Provider>
  );
}
...
function AnchoredToastProvider({
  ...
  activeThreadRef = null,
  ...props
}: Toast.Provider.Props & { activeThreadRef?: ScopedThreadRef | null }) {
  return (
    <Toast.Provider toastManager={anchoredToastManager} {...props}>
      {children}
      <AnchoredToasts activeThreadRef={activeThreadRef} />
    </Toast.Provider>
  );
}

function AnchoredToasts({ activeThreadRef }: { activeThreadRef: ScopedThreadRef | null }) {
  const { toasts } = Toast.useToastManager<ThreadToastData>();
