...
// on the picker's rail. Only Claude Code has a mark; the others show their
// initials until an adapter for them lands.
// Claude mark adapted from pingdotgg/t3code apps/web/src/components/Icons.tsx at be7796d (MIT).
import type { ComponentProps } from "react";
import { cn } from "../../lib/utils";

function ClaudeMark({ className, ...props }: ComponentProps<"svg">) {
  return (
    <svg {...props} preserveAspectRatio="xMidYMid" viewBox="0 0 256 257" className={cn("fill-current", className)} aria-hidden>
...
export function HarnessMark({ harness, label, className }: { harness: string; label: string; className?: string }) {
  if (harness === "claude") return <ClaudeMark className={cn("size-3.5 shrink-0", className)} data-harness-mark={harness} />;
  return (
