...
describe("the composer's picks as they come back off local storage", () => {
  ...
  it("reads back the picks and the thread each of them was made in", async () => {
    const state = await stored({ byWorkspaceId: { ws_a: { model: "claude-opus-5", contextWindow: "1m" } }, pickedOn: { ws_a: { model: "t1", contextWindow: "t2" } } });
    expect(state.byWorkspaceId).toEqual({ ws_a: { model: "claude-opus-5", contextWindow: "1m" } });
    expect(state.pickedOn).toEqual({ ws_a: { model: "t1", contextWindow: "t2" } });
  });
  ...
  it("drops a thread record of the wrong shape rather than handing the pickers a value that is not a thread", async () => {
    ...
    const odd = { ws_a: { model: 7 }, ws_b: { model: null }, ws_c: { model: "" }, ws_d: { model: { id: "t1" } }, ws_e: { model: "t2", effort: "t3", contextWindow: "t4" } };
    expect((await stored({ byWorkspaceId: {}, pickedOn: odd })).pickedOn).toEqual({ ws_e: { model: "t2", contextWindow: "t4" } });
  ...
  it("a pick names the thread it was made in, each pick on its own, and nothing else is named", () => {
    ...
    // Effort and access are not scoped to a thread yet, which is #643; they name no thread.
    pick("ws_a", "effort", "low", "t3");
    pick("ws_a", "harness", "codex", "t3");
    expect(useComposerOptionsStore.getState().pickedOn).toEqual({ ws_a: { model: "t1", contextWindow: "t2" } });
    ...
    expect(useComposerOptionsStore.getState().pickedOn).toEqual({ ws_a: { model: "t9", contextWindow: "t2" } });
