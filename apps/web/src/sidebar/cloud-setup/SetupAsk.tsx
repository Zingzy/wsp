    ...
    <SetupScreen k="ask" counter={counter} headline={ASK_HEADLINE} top={setup.pricing !== null ? initForkLine(setup.pricing.size) : words.top} refusal={refusal} primary={{ word: CLOUD_SETUP_WORDS.screen.build, onPress: build, disabled: !ready, focus: false, title: words.needsName }} secondary={{ word: CLOUD_SETUP_WORDS.screen.back, onPress: onBack }}>
      ...
      <div className="mt-6 flex w-full flex-col gap-2" onDragOver={e => (carriesFiles(e.dataTransfer) ? e.preventDefault() : undefined)} onDrop={drop} data-k="folder-row">
        ...
        <div className="relative w-full">
          <Input id="setup-first-folder" size="compact" value={folder} placeholder={FOLDER_GHOST} spellCheck={false} onChange={e => setFolder(e.target.value)} onBlur={() => onKeep({ name, folder })} onKeyDown={e => (e.key === "Enter" && ready ? build() : undefined)} className={cn(LONE_FIELD, "min-w-0", canChoose && "[&_input]:pr-[96px]")} />
          {canChoose ? (
