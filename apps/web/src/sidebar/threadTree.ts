...
/** One project of the sidebar: the record the host holds for it, and the workspaces of it a person started
 * themselves. A workspace an agent forked is not here; it is drawn under the thread that forked it. */
...
/** The projects the sidebar draws, in the order the host holds them, each with its own workspaces under it. A
 * project every one of whose workspaces an agent forked still has its header and its plus: the workspaces are
 * drawn under their threads and the project is still where the next piece of work starts. A workspace whose
 * project the host's list does not carry keeps a project of its own off its own record, so a list that has not
 * arrived, or a workspace of a project another computer holds, never loses its row. */
export function projectGroups(recorded: ReadonlyArray<ProjectView>, rows: ReadonlyArray<SidebarProjectSnapshot>): ProjectGroup[] {
  ...
  for (const row of rows) {
    ...
    if (row.workspace.parentThreadId === undefined) group.workspaces.push(row);
  }
  return [...groups.values()];
}

/** The workspaces one thread's own agent forked, in the order the sidebar draws workspaces. A fork is a copy of
 * the whole computer with its own branch, so its row carries the same three lines a top-level row does, one step
 * in under the thread that asked for it. */
export function forkedWorkspaces(rows: ReadonlyArray<SidebarProjectSnapshot>, openerThreadId: string): SidebarProjectSnapshot[] {
  return rows.filter(row => row.workspace.parentThreadId === openerThreadId);
}
