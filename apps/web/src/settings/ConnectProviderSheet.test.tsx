...
const rowsWith = (save: ProviderRow["save"]): ProviderRow[] => [
  { id: "box", name: "ASCII", what: "always on, naps to $0", fromUsdPerHour: 0.018, placeholder: "ascii_…", console: "https://ascii.dev", trial: true, ...(save === undefined ? {} : { save }) },
  { id: "solari", name: "Solari", what: "in memory, wakes fast", fromUsdPerHour: 0.11, placeholder: "slr_live_...", console: "https://console.getsolari.com" },
];

const mount = (rows: readonly ProviderRow[]) => {
  useStore.setState({ api: { subscribe: () => () => {} } as unknown as Api });
...
describe("Connect a provider", () => {
  it("picks from the provider table, each with what it is and its cheapest rate", async () => {
    ...
    expect(rows).toEqual([
      ["box", "ASCIIalways on, naps to $0 · from $0.018/h"],
      ["solari", "Solariin memory, wakes fast · from $0.11/h"],
    ]);
