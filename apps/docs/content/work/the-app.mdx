...
`Ask anything, or / for commands`. Enter sends, Shift-Enter is a newline, `/` at the start opens the agent's own slash commands, read off its binary on that machine, less the ones that work only in the agent's own terminal (`/login`, `/model`, `/help` and their kind). Typing one of those sends nothing; the line above the box names the wsp control that does the same thing: the Machine tab for signing a machine in, your own terminal for this computer, the pickers under the box for the model and the access mode, wsp's settings and docs for the rest. A slash command the agent never announced still goes as text.
