...
The host serves the app at `http://127.0.0.1:4400`; the desktop app is that same host in a window, with an Electron process on top of it. The browser `wsp init` opens is yours; another browser on this computer pairs as a device, with a code from `wsp host pair`. Three regions: workspaces and their threads on the left, the open thread in the middle, and the machine on the right.
...
| `mod+j` | toggle the terminal drawer under the thread; with no workspace selected, a shell on this computer in your home folder |
...
<Callout type="info">
  There is no in-app theme toggle; the app ships dark. There is no auto-updater. When Settings > About names a release newer than the app, Get downloads that release's bundle into Downloads and keeps it only if its sha256 and size match what the release publishes; Quit and open checks the sum again, opens it and quits the app, so the new one is dragged over an app that is not running. When only the host is behind, on a page served by a host on another computer, or in a browser tab, Get opens the release's page instead. The bundles are unsigned, so macOS asks once for the new app: Privacy & Security, Open Anyway. When the app's files were replaced under a running copy, Restart host takes Get's place and relaunches the app on them.
</Callout>
