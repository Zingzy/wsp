...
The host serves the app at `http://127.0.0.1:4400`; the desktop app is that same host in a window, with an Electron process on top of it. Three regions: workspaces and their threads on the left, the open thread in the middle, and the machine on the right.
