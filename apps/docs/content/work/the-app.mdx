...
<Callout type="info">
  There is no in-app theme toggle; the app ships dark. There is no auto-updater. When Settings > About names a newer release, Get downloads that release's bundle into Downloads and keeps it only if its sha256 matches the one the release publishes; Quit and open then opens it and quits the app, so the new one is dragged over an app that is not running. On a page served by a host on another computer, or in a browser tab, Get opens the release's page instead. The bundles are unsigned, so macOS asks once for the new app: Privacy & Security, Open Anyway.
</Callout>
