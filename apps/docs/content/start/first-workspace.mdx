...
Six screens, every row ticked from what this computer runs. Enter through them, finish the sign-ins in your browser as the build reaches them, and wait for `Golden v1 sealed.` The first seal also forks one workspace for you, named `first`, starts serving the app at `http://127.0.0.1:4400` and opens it on that workspace at an address carrying a code, `http://127.0.0.1:4400/#w/<id>/c/<code>`, which lets that browser in as you.
