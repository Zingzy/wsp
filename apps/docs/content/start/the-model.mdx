...
The host is the process on your computer that owns everything: the machines, the keys, the app it serves at `http://127.0.0.1:4400`, and the runtime behind it. The first command that needs one starts it, `wsp up` serves one you want to watch in a terminal, `wsp up --service` keeps one up across logins, and the desktop app is the same host in a window. The command line and the MCP server are thin clients of the host, which is why what an agent does through them shows in your sidebar.
