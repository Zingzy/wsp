...
| `resize` | a machine that already exists is given a size it was not made at, which the resize verb and the app's Upgrade button both read | no, the Starter plan clamps every machine to 2 vCPU |
