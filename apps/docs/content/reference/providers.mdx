...
| `diskSnapshots` | a running machine's disk is copied into an image the account holds, which is what a version and a project golden are sealed as; without it a workspace cannot be snapshotted | yes, of a machine that was never resumed |
