...
| flag | meaning | Solari |
| --- | --- | --- |
| `liveCloneForks` | what a fork of an image comes up as: with the processes still running where the image holds memory, or booted cold with the agents started again where it holds only a disk. The provider table publishes it; no verb reads it, since each verb reads the road its own move needs | yes |
...
| `resize` | a machine is asked for at a size another was not made at. Half of the resize road, since a resize replaces the machine first, so the verb and the app's Upgrade button both read this flag and `replacesMachine` together | no, the Starter plan clamps every machine to 2 vCPU |
| `replacesMachine` | a fresh fork of the image behind a machine stands in for it and the workspace goes on, its vaulted files carried over: the road a rebuild and an image move both read | yes |
...
| `containers` | the guest runs containers; without it services install natively | no, the guest kernel has no overlayfs or netfilter, so a container engine does not run |
...
| `diskSnapshots` | a running machine's disk is copied into an image the provider keeps, which is what a version and a project golden are sealed as; without it a workspace cannot be snapshotted | yes, of a machine that was never resumed |
