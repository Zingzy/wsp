...
Every verb takes `--state <path>`, the state file the host serves. The default is `~/.wsp/state.json`, or `./.wsp/state.json` in a checkout of wsp itself, which its own root `package.json` names. The MCP server and the command line refuse a host of another version in one line; restart it with `wsp up`.
