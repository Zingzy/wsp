...
Everything sits beside the state file. The default home is `~/.wsp`; `WSP_HOME` moves it, and `--state <path>` on any verb names another state file. In a checkout of wsp itself, which its own root `package.json` names, the default state file is `./.wsp/state.json` instead, so a repository's own setup stays apart from your personal one.
...
| `~/.wsp/runs/` | a turn running on this computer: its script, log and exit code, so a host restart does not cut it; mode 700 |
...
| `~/.wsp/roots` | the folders the daemon for this computer's workspace may browse |
| `~/.wsp/inbox` | files handed to a turn on this computer, for that daemon to pick up |
