...
What the relay knows is the whole of what it knows: which boxes are on your account, what you called each one, and the hostname each answers at. It holds no pairing code, no device token and no byte of anything your agents say. Its database has no column for them. The code still comes from `wsp pair` on the box and travels to you by whatever channel you like.

`wsp relay unlink` on the box takes it off your account, stops the tunnel and deletes the hostname. `wsp up --no-relay` serves without the tunnel for one run.
...
The API token needs Cloudflare Tunnel write on your account, and DNS write on one zone if you want managed hostnames. `RELAY_ZONE` and `RELAY_ZONE_ID` in `wrangler.jsonc` name that zone; leave them empty and every box gets a quick tunnel instead. The GitHub OAuth app is yours: its callback is `https://<your relay>/link/callback`, and whoever signs in gets their own account with their own boxes.
