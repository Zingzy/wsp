...
Every line here is one word in: `wsp host --help` prints them. Whichever road you take, pairing is the gate. `wsp host pair` on the box prints the code, `wsp host devices` lists the computers that took one, and `wsp host devices revoke <id>` takes one back out.
