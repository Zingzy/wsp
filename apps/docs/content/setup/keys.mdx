...
One request does leave, and it carries nothing of yours. Thirty seconds after it starts, every six hours after that and when you open Settings > About, the host asks api.github.com for the latest wsp release, never more than once in ten minutes. The request names wsp's own version as its user agent and sends nothing else. The answer is kept in `release.json` beside the state file, which `wsp status` and `wsp doctor` read without asking again. To turn it off, set `WSP_UPDATE_CHECK=0` in the host's environment or on a line of `~/.wsp/.env`, which is the one the desktop app reads.
