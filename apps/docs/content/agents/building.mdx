...
1. **One workspace with the repo imported.** `wsp import <workspace> <folder>` on a fresh workspace, the app's import dialog, or `wsp new <name> --from <project golden>` from a snapshot taken after an import. `wsp threads <workspace>` shows what is on it.
...
- An agent on the person's computer starts threads with `--detach` and reads their ends with `threads wait`, one per call. `run` without it blocks for the whole turn.
...
- `run` and `send` return when the reply is complete, not when the process exits minutes later. A send that meets that gap is refused in words naming the thread.
