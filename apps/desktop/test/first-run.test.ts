...
// The first launch's screen, driven as the shell drives it: the page it ships
// as, parsed and run, with the bridge the preload exposes faked. One rule, one
...
async function open(agents: unknown[] = AGENTS, opts: { refusedScan?: number; refusedTools?: string[] } = {}): Promise<Screen> {
  const asks: Asks = { install: [], finish: 0 };
  let scans = 0;
...
describe("the first launch's screen", () => {
  ...
  it("has one keycap, Open wsp, which gives the found agents the tools and then asks the shell to record this Mac and open the app on it", async () => {
    ...
    expect(screen.window.document.querySelectorAll("#welcome button.primary")).toHaveLength(1);
