...
// dot grid, one mono line. Nothing else.
import { mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { fileURLToPath } from "node:url";
import { chromium } from "playwright";
import { DMG_ICONS, DMG_LINE, DMG_WINDOW } from "./dmg-layout.mjs";

const out = join(fileURLToPath(new URL("..", import.meta.url)), "build");

const LINE = "your setup, on cloud machines";

/** The app's light side: zinc-25 as the ground, zinc-500 as the muted ink, its primary as the glow's hue. */
